import { Backpack, Player, Pokemon } from "../../../interfaces.ts";
import { allPokemon, setAlert } from "../game.ts";
...
export function createBackpack(player: Player) {
  const backpack: Backpack = {
    ...
    closeBtn: document.getElementById("backpack_closeBtn"),
    backBtn: document.getElementById("backpack_backBtn"),
  ...
  backpack.backpackIcon.addEventListener("click", () => {
    if (!player.isInEvent) {
  ...
  backpack.closeBtn.addEventListener("click", () => {
    backpack.closeAllEvents();
  });
  backpack.backBtn.addEventListener("click", () => {
...
function closeAllEvents(this: Backpack) {
  this.closeMenu();
  this.menuEvents.forEach((menuEvent: any) => {
...
function backbuttonPressed(this: Backpack) {
...
function openMenu(this: Backpack) {
...
function closeMenu(this: Backpack) {
  this.player.isInEvent = false;
...
// ========= events ======== //
// ========================= //
function openEvent(this: Backpack, event: any) {
  this.openMenu();
  if (this.currentMenu) {
    this.currentMenu.close(this.currentMenu.event);
    this.previousMenu.push(this.currentMenu);
  }
  this.currentMenu = this.menuEvents.find((menuEvent: any) => menuEvent.event === event);
  this.currentMenu.event.style.display = "block";
  const backpackTitle: any = document.getElementById("backpackTitle")
  backpackTitle.innerHTML = this.currentMenu.title;
}
// ========= main menu ======== //
function openMainMenu(this: Backpack, event: any) {
  this.openEvent(event);
  this.backBtn.style.visibility.style.visibility = "hidden";
}
function closeMainMenu(this: Backpack, event: any) {
  event.style.display = "none";
  this.backBtn.style.visibility = "visible";
}

// ========= pokedex ======== //
function openPokedexEvent(this: Backpack, event: any) {
  this.openEvent(event);
...
function createPokemonList(this: Backpack, table: any, pokemonList: Pokemon[], rowClickFunction: Function) {
  table.innerHTML = "";
...
function pokedexRowClick(this: Backpack, pokemon: Pokemon) {
  const menuEvent = this.menuEvents[1];//this.menuEvents.find(event=> event.title === "stats");
  menuEvent.open(menuEvent.event, pokemon);
}
function compareLeftRowClick(this: Backpack, pokemon: Pokemon) {
...
function compareRightRowClick(this: Backpack, pokemon: Pokemon) {
  const menuEvent = this.menuEvents[2];//this.menuEvents.find(event=> event.title === "stats");
...
// ========= pokedex detail ======== //
function openDetailEvent(this: Backpack, event: any, pokemon: Pokemon) {
  this.openEvent(event);
  if (!pokemon) {
    return;
  }
  const pokedexDetails: any = document.getElementById("pokedex_detail");
...
function closeDetailsEvent(event: any) {
  event.style.display = "none"
}

// ========= compare ======== //
function openCompareEvent(this: Backpack, event: any) {
  this.openEvent(event);
...
function openCompareSide(compareSide: any, pokemon: Pokemon) {
...
function updateCompareSides() {
  ...
  const menuCompare: any = document.getElementById("menu_compare")
  const sides: any = menuCompare.getElementsByClassName("compare_sides");
...
// ========= who's that pokemon ======== //
function openWhosThatEvent(this: Backpack, event: any, pokemon: Pokemon) {
  this.openEvent(event);
  ...
  const autoFilloptions: any = document.getElementById("whosThat_autocompleteOptions");
...
function closeWhosThatEvent(event: any) {
  event.style.display = "none";
  const input = event.getElementsByTagName("input")[0];
  const button = event.getElementsByTagName("button")[0];
...
// ========= battle ======== //
export function openBattleEvent(this: Backpack) {
...
export function closeBattleEvent(this: Backpack) {
  const event = this.menuEvents[4]//this.menuEvents.find(event => event.title === "gevecht");
...
// ========= capture ======== //
export function openCaptureEvent(this: Backpack, player: Player, pokemon: Pokemon) {
  const event = this.menuEvents[5]//this.menuEvents.find(event => event.title === "vangen");
  this.openEvent(event.event);
}

export function closeCaptureEvent(this: Backpack) {
  const event = this.menuEvents[5]//this.menuEvents.find(event => event.title === "vangen");
  event.event.style.display = "none";
  this.closeAllEvents();
}

function openAccountEvent(this: Backpack, event: any) {
  this.openEvent(event);
}

function closeAccountEvent(event: any) {
  event.style.display = "none";
}